...
var timerFreezeNoti = new Push("Unaccepted bets timer frozen");
var betFreezeNoti = new Push("Betting was temporarily stopped");
var firstHalfEndNoti = new Center("First half ended");
var matchEndNoti = new Center("Match ended");
...
function setTableTooltip(event, direction, secondDirection) {
    ...
    switch (event) {
        case "goal":
            message = "Goal on the<br>";
            break;

        case "fine":
            message = "Free Kick on the<br>";
            break;

        case "offside":
            message = "Offside on the<br>";
            break;

        case "goalKick":
            message = "Goal Kick on the<br>";
            break;

        case "out":
            message = "Throw-in on the<br>";
            break;

        case "corner":
            message = "Corner on the<br>";
            break;
...
function setChipValue(value) {
    if (value > 10000) {
        ...
        new Push("You exceed the maximum bet!").temporarily();
        return;
    } else if (value <= 0) {
        activeBet.classList.remove("active");
        new Push("Incorrect input").temporarily();
        return;
    }
...
accept.onclick = function (e) {
    ...
    sendPost("/api/bets/add", req, function (data) {
        toggleDisableBut();
        if (data.error === null) {
            new Center("The bet was fixed").temporarily();
            ...
            new Center("The bet was not fixed").temporarily();
        }
    }).fail(function () {
        toggleDisableBut();
    })
...
cancel.onclick = function (e) {
    canselPari();
    new Center("Bet was reset").temporarily();
};
    ...
    sendPost("/api/matches/info", {id: matchId}, function (data) {
        ...
        if (match.status === "ENDED") {
            ...
            if (timerFreezeNoti.isActive()) {
                ...
                new Push("Unaccepted bets timer is unfrozen").temporarily();
            } else if (betFreezeNoti.isActive()) {
                betFreezeNoti.hide();
                new Push("Betting resumed").temporarily();
